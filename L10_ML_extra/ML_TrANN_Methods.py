...
def run_quick_test():
    ...
    # Train the model
    nn_model.fit(X_train_tf, y_train_tf, epochs=50, batch_size=1000, verbose=0)
    ...
    print(
        f"Tree structure - Nodes: {tree.tree_.node_count}, Max depth: {tree.tree_.max_depth}"
    )
    ...
    # Train the model
    treenn1_model.fit(X_train_aug_tf, y_train_tf, epochs=50, batch_size=1000, verbose=0)
...
def run_asset_pricing_with_trann(data_path="ML_sample.dta", seed=42):
    ...
    retdata = retdata.astype(
        {"permno": "int", "year": "int", "month": "int", "ym": "int"}
    )
    retdata = retdata[
    ...
    retdata["exret_winsor"] = retdata.groupby("ym")["exret"].transform(
        winsorize, q=0.01
    )
    ...
    for i, hp_i in enumerate(hp_ls):
        ...
        # Train the model
        NN.fit(
            x=X_train_tf, y=Y_train_tf, batch_size=hp_i[2], epochs=hp_i[3], verbose=0
        )
    ...
    feature_importance = pd.DataFrame(
        {"feature": CharsVars, "importance": tree.feature_importances_}
    ).sort_values("importance", ascending=False)
    print("\nTREENN1 Tree Feature Importance:")
    print(feature_importance)

    # Normalize tree predictions to [0,1] range
    tree_preds_train_norm = (tree_preds_train - np.min(tree_preds_train)) / (
        np.max(tree_preds_train) - np.min(tree_preds_train)
    )
    tree_preds_valid_norm = (tree_preds_valid - np.min(tree_preds_train)) / (
        np.max(tree_preds_train) - np.min(tree_preds_train)
    )
    tree_preds_test_norm = (tree_preds_test - np.min(tree_preds_train)) / (
        np.max(tree_preds_train) - np.min(tree_preds_train)
    )
    ...
    for i, mid in enumerate(avg_models_id):
        if i == 0:
            Y_pred = (
                np.reshape(
                    TREENN1_ls[mid](X_test_augmented_tf, training=False).numpy(), [-1]
                )
                / navg
            )
        else:
    ...
    for i in range(5):
        ...
        tree_preds_train[:, i] = (tree_preds_train[:, i] - min_val) / (
            max_val - min_val
        )
        tree_preds_valid[:, i] = (tree_preds_valid[:, i] - min_val) / (
            max_val - min_val
        )
    ...
    for i, mid in enumerate(avg_models_id):
        if i == 0:
            Y_pred = (
                np.reshape(
                    FONN1_ls[mid](X_test_augmented_tf, training=False).numpy(), [-1]
                )
                / navg
            )
        else:
    ...
    for i, hp_i in enumerate(hp_ls):
        ...
        if r2 > best_valid_r2:
            ...
            for j in range(5):
                print(
                    f"Tree {j+1}: R² = {tree_r2_scores[j]:.4f}%, Weight = {weights[j]:.4f}"
                )
    ...
    try:
        ...
        if os.path.exists(data_path):
            ...
            with open(report_filename, "w") as f:
                f.write("=" * 80 + "\n")
                f.write(
                    "ASSET PRICING WITH MACHINE LEARNING METHODS - DETAILED RESULTS\n"
                )
                f.write("=" * 80 + "\n\n")
                ...
                for seed in seeds:
                    ...
                    if "TREENN1" in results and "NN" in results:
                        improvement = (
                            (results["TREENN1"] - results["NN"]) / results["NN"] * 100
                        )
                        f.write(f"\nTREENN1 improvement over NN: {improvement:.2f}%\n")
                ...
                for seed in seeds:
                    ...
                    if "TREENN1" in results and "NN" in results:
                        ...
                        f.write(
                            f"Seed {seed}: TREENN1 improvement over NN: {improvement:.2f}%\n"
                        )
