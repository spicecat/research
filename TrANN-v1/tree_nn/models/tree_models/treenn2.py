...
class TREENN2:
    def __init__(self, input_dim, hidden_dim, output_dim, X_train, y_train):
        ...
        self.tree_hidden = DecisionTreeRegressor(max_depth=5, random_state=0).fit(
            X_train, y_train
        )
        ...
        self.weights_output = np.random.randn(hidden_dim + 1, output_dim) * np.sqrt(
            2.0 / hidden_dim
        )
    ...
    def train(self, X, y, epochs, learning_rate, gradient_clip=1.0):
        for epoch in range(epochs):
            ...
            hidden_error = np.dot(output_error, self.weights_output.T)[:, :-1] * (
