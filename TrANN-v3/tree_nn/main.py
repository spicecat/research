...
def main():
    ...
    try:
        # Initialize wandb
        wandb.init(
            project="tree-neural-networks", name="model-comparison", config=MODEL_PARAMS
        )
        ...
        models_to_evaluate = {
            ...
            "Forest Models": [("FONN1", FONN1), ("FONN2", FONN2), ("FONN3", FONN3)],
        }
            ...
            for name, model in model_list:
                try:
                    ...
                    if name == "FONN3":
                        ...
                        logger.info(
                            f"Using reduced learning rate for FONN3: {current_lr}"
                        )
                        logger.info(f"Using reduced alpha for FONN3: {alpha_value}")
                    elif name == "FONN2":
                        # FONN2 doesn't have an alpha parameter in its constructor
                        logger.info(
                            f"FONN2 uses trees in the hidden layer with standard learning rate: {current_lr}"
                        )
...
def train_and_evaluate_model(
    name, model, X_train, X_test, y_train, y_test, learning_rate, gradient_clip=1.0
):
    ...
    logger.info(
        f"Applying regularization: L2 strength={MODEL_PARAMS['l2_strength']}, Dropout rate={MODEL_PARAMS['dropout_rate']}"
    )
        ...
        if hasattr(model, "train_with_reg"):
            ...
            if MODEL_PARAMS["l2_strength"] > 0:
                if hasattr(model, "weights_hidden"):
                    model.weights_hidden -= (
                        learning_rate
                        * MODEL_PARAMS["l2_strength"]
                        * model.weights_hidden
                    )
                if hasattr(model, "weights_output"):
                    model.weights_output -= (
                        learning_rate
                        * MODEL_PARAMS["l2_strength"]
                        * model.weights_output
                    )
                ...
                if MODEL_PARAMS["regularize_trees"]:
                    if any(
                        hasattr(model, attr)
                        for attr in ["trees_output", "trees_hidden", "trees_input"]
                    ):
                        logger.info(
                            f"Skipping tree regularization for {name} - feature_importances_ is read-only"
                        )
...
def evaluate_mlp(X_train, X_test, y_train, y_test):
    ...
    try:
        ...
        logger.info(
            f"PureMLP with L2 regularization (alpha={MODEL_PARAMS['l2_strength']})"
        )

        # Apply dropout to the training data
        if MODEL_PARAMS["dropout_rate"] > 0:
            logger.info(
                f"Applying dropout with rate {MODEL_PARAMS['dropout_rate']} to MLP input"
            )
            X_train_dropout = np.copy(X_train)
            for i in range(X_train.shape[0]):
                # Create dropout mask
                dropout_mask = np.random.binomial(
                    1, 1 - MODEL_PARAMS["dropout_rate"], size=X_train.shape[1]
                )
                # Ensure at least 3 features are active
                if np.sum(dropout_mask) < 3:
                    random_indices = np.random.choice(
                        X_train.shape[1], 3, replace=False
                    )
