...
ACTIVATIONS = {"relu": lambda x: np.maximum(0, x), "tanh": lambda x: np.tanh(x)}
DERIVATIVES = {
...
class Ensemble(BaseEstimator):
    ...
    def predict(self, X):
        ...
        tree_predictions = np.column_stack([tree.predict(X) for tree in self])

        # Compute feature importance weights for each tree
        tree_importances = [tree.feature_importances_ for tree in self]
        ...
        tree_weights = np.array(
            [
                (
                    ...
                    else np.ones_like(importances)
                )
                for importances in tree_importances
            ]
        )
...
class MLP(BaseEstimator):
    ...
    def _initialize(self):
        hidden_dim = (
            ...
            if hasattr(self.hidden_dim, "__iter__")
        ...
        self.intercept_grads = [np.empty(n_fan_out_) for n_fan_out_ in layer_units[1:]]
    ...
    def _forward(self, X):
        ...
        for i in range(self.n_layers_ - 1):
            ...
            if i + 1 != self.n_layers_ - 1:
                ...
                activations[i + 1] = ACTIVATIONS[self.activation](activations[i + 1])
    ...
    def _backward(self, y, activations):
        ...
        for i in range(self.n_layers_ - 1, 0, -1):
            ...
            self.coef_grads[i - 1] = np.dot(activations[i - 1].T, loss) / y.shape[0]
...
class FONN1(MLP, Ensemble):
    def __init__(
        ...
        *,
        ...
        self.intercept_grads = []
        self._initialize()
        self.trees = Ensemble(num_trees_input, max_depth)
    ...
    def _initialize(self):
        ...
        layer_units = [
            self.input_dim + self.num_trees_input,
            *hidden_dim,
            self.output_dim,
        ]
        self.n_layers_ = len(layer_units)
    ...
    def _forward(self, X):
        ...
        input_tree_outputs = np.column_stack([tree.predict(X) for tree in self.trees])
    ...
    def get_params(self, deep):
        ...
        params.update({"num_trees_input": self.trees.num_trees})
        return params
...
class FONN2(MLP, Ensemble):
    def __init__(
        ...
        *,
        ...
        self.intercept_grads = []
        self._initialize()
        self.trees = Ensemble(num_trees_hidden, max_depth)
    ...
    def get_params(self, deep):
        params = super().get_params(deep)
        params.update({"num_trees_hidden": self.trees.num_trees})
        return params
...
class TREENN2(FONN2):
    def __init__(self, input_dim, hidden_dim, output_dim, **kwargs):
        super().__init__(
            input_dim, hidden_dim, output_dim, num_trees_hidden=1, **kwargs
        )
