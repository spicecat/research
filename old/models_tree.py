...
class FONN1:
    def __init__(self, input_dim, hidden_dim, output_dim, num_trees_input, X_train, y_train):
        self.input_dim = input_dim
        self.hidden_dim = hidden_dim
        self.output_dim = output_dim
        self.num_trees_input = num_trees_input

        # Initialize decision trees for the input layer and fit them
        # self.trees_input = [DecisionTreeRegressor(max_depth=5, random_state=i).fit(X_train, y_train) for i in range(num_trees_input)]
        self.trees_input = bootstrap_trees(X_train, y_train, R=num_trees_input, max_depth=5)

        # Initialize weights and biases for the hidden layer
        self.weights_hidden = np.random.randn(input_dim + num_trees_input, hidden_dim) * 0.01
    ...
    def forward(self, X):
        ...
        input_tree_outputs = np.column_stack([tree.predict(X) for tree in self.trees_input])
        combined_input = np.hstack((X, input_tree_outputs))

        # Compute hidden layer activations
        self.z_hidden = np.dot(combined_input, self.weights_hidden) + self.bias_hidden
    ...
    def backward(self, X, y, output, learning_rate):
        ...
        hidden_error = np.dot(output_error, self.weights_output.T) * (1 - self.a_hidden ** 2)  # Tanh derivative
        combined_input = np.hstack((X, np.column_stack([tree.predict(X) for tree in self.trees_input])))
...
class TREENN1:
    def __init__(self, input_dim, hidden_dim, output_dim, X_train, y_train):
        ...
        # Initialize the single decision tree for the input layer and fit it
        # self.tree_input = DecisionTreeRegressor(max_depth=5, random_state=0).fit(X_train, y_train)
        self.tree_input = bootstrap_trees(X_train, y_train, R=1, max_depth=5)[0]
    ...
    def backward(self, X, y, output, learning_rate):
        ...
        # Compute hidden layer error and gradients
        hidden_error = np.dot(output_error, self.weights_output.T) * (1 - self.a_hidden ** 2)  # Tanh derivative
        combined_input = np.hstack((X, self.tree_input.predict(X).reshape(-1, 1)))
        d_weights_hidden = np.dot(combined_input.T, hidden_error) / X.shape[0]
        d_bias_hidden = np.mean(hidden_error, axis=0)
...
class TREENN2:
    ...
    def forward(self, X, y_batch=None):
        ...
        if y_batch is not None:
            # Generate tree output for the hidden layer using batch labels
            hidden_tree_output = self.tree_hidden.fit(self.a1, y_batch).predict(self.a1).reshape(-1, 1)
            # hidden_tree_output = bootstrap_trees(self.a1, y_batch, R=1, max_depth=5)[0].predict(self.a1).reshape(-1, 1)
        else:
            # If y_batch is None, use the tree without fitting
            hidden_tree_output = self.tree_hidden.predict(self.a1).reshape(-1, 1)

        self.combined_hidden = np.hstack((self.a1, hidden_tree_output))
