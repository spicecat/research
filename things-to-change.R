...
#save each tree model in an array
#generate the tree models using new algorithm to control number of terminal nodes
# update instead of terminal threshold, number of terminal nodes
k_fold_mse = function(dataset, k, terminal_threshold, dependent_variable) {
  ...
  for(i in 1:k){
    ...
    regressor <- decision_tree_regressor$new(min_samples_leaf = terminal_threshold)
    regressor$fit(train)
    regressor$fit(train, dependent_variable)
    mse_total_a <- mse_total_a + regressor$mean_squared_error(testData, dependent_variable)
  }
  dataset <- dataset_original
  mse_total_a / k
}
# match function to take in i as number of terminal nodes, not threshold
plot_mse = function(n, dataset, k, dependent_variable) {
  mse_values <- 1:n;
  for(i in 1:n) {
    temp <- k_fold_mse(boston, k, i, "medv")
    mse_values[i] = temp
  }
  plot(1:n, mse_values)
}
...
print("MSE")
print(regressor$mean_squared_error(boston[-as.numeric(rownames(train)),], "medv"))
print("k-fold test")
print(k_fold_mse(boston[as.numeric(rownames(train)),], 10, 5, "medv"))
plot_mse(10, boston[as.numeric(rownames(train)),], 10, "medv")
#find lowest mse, calculate standard deviation of all mse's
# 1 standard deviation to left of minimum value
#then generate model using all training data, with that number of terminal nodes
#then find test mse for testing data
regressor$summarize()
