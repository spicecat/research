...
tree <- function(dataset, dependent_variable, explanatory_variables, threshold, train) {
  ...
  if (length(subset(dataset, eval(parse(text = best_var)) < best_rule)[, dependent_variable]) > threshold) {
    ...
    print(h)
    print("then")
    tree(subset(dataset, eval(parse(text = best_var)) < best_rule)[, 1:num], dependent_variable, explanatory_variables, threshold, seq_along(subset(dataset, eval(parse(text = best_var)) < best_rule)[, dependent_variable]))
  } else {
    print("if less than")
    print(h)
    print("estimate is")
