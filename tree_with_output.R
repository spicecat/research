...
decision_tree_regressor <- setRefClass("decision_tree_regressor",
  ...
  methods = list(
    ...
    summarize = function() {
      rules1 <- capture.output(print(root))
      rules <- capture.output(cat(rules1))
      node_count <- str_count(rules, pattern = "score=") - str_count(rules, pattern = "] score=")
      rule_errors <- str_extract_all(rules, "\\d score=\\d*\\.\\d*, samples=\\d")[[1]]
      sample_count <- 0
      error_total <- 0
      for (value in rule_errors) {
        sample_count_value <- strtoi(substring(value, nchar(value), nchar(value)))
        sample_count <- sample_count + sample_count_value
        error_total <- error_total + (as.double(substring(value, 9, nchar(value) - 11)) * sample_count_value)
      }
      residual_mean_deviance <- error_total / (sample_count - node_count)
      unique_var <- unique(str_extract_all(rules, "[:alpha:]* <")[[1]])
      print("variables used")
      for (value in unique_var) {
        print(substring(value, 1, nchar(value) - 2))
      }
      print("number of nodes")
      print(node_count)
      print("residual mean deviance")
      print(residual_mean_deviance)
      print("sample count")
      print(sample_count)
